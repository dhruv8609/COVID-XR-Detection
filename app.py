...
UPLOAD_FOLDER = './flask app/static/images'
...
@app.route('home')
def home():
   return {"content": "This is the home pallete. Go to '/uploaded_xr' or '/uploaded_ct'"}


@app.route('/uploaded_xr', methods=['POST', 'GET'])
def uploaded_result():
   if request.method == 'POST':
      if 'file' not in request.files:
         return {"results": "No file part"}
      file = request.files['file']
      if file.filename == '':
         return {"results": "No selected file"}
      if file:
   ...
   image = cv2.imread('./flask app/static/images/upload_chest.jpg')
   ...
   if probability[0] > 0.5:
      ...
      resnet_chest_pred = str(
          '%.2f' % ((1 - probability[0]) * 100) + '% NonCOVID')
   ...
   return {'results' : {'resnet_chest_pred' : resnet_chest_pred, 'vgg_chest_pred':vgg_chest_pred, 'inception_chest_pred':inception_chest_pred, 'xception_chest_pred': xception_chest_pred}}
...
def uploaded_ct():
   if request.method == 'POST':
      if 'file' not in request.files:
         return {'results' : 'No file found'}
      file = request.files['file']
      if file.filename == '':
         return {'results' : 'No selected found'}
      if file:
   ...
   image = cv2.imread('./flask app/static/images/upload_ct.jpg')
   image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
   image = cv2.resize(image, (224, 224))
   image = np.array(image) / 255
   image = np.expand_dims(image, axis=0)
   ...
   return {'results' : {'resnet_ct_pred' : resnet_ct_pred , 'vgg_ct_pred': vgg_ct_pred, 'inception_ct_pred':inception_ct_pred, 'xception_ct_pred'=xception_ct_pred}}
